...
> Built because I got tired of paying $48/year just to autofill a password.
...
| Account signup required | ❌ Yes - email + verify | ❌ Yes - email + verify | ❌ Yes - email + verify | ❌ Yes - email + verify | ❌ Yes - Proton account | ✅ No - install and go |
| **Hacked / compromised / leaked** | No major incident known | ⚠️ April 2026: npm CLI supply-chain compromise (~334 installs, no vault data accessed) [^bw2026] | ❌ **May 2026: encrypted vaults of <20 personal-plan accounts stolen via device-registration 2FA brute-force (disclosed June 2026)** [^dl2026] | ❌ **2022: backup DB stolen via two-stage employee laptop compromise; customer names, emails, phone numbers, and vault URLs all left unencrypted; 1.6M UK users hit; £1.2M (~$1.5M) ICO fine Dec 2025** [^lp2022] | No major incident known | ✅ **None - no server to breach** |
...
| Account metadata held off-device | Email, billing, devices | Email, devices | ⚠️ Email, devices (cloud breached 2026) | ⚠️ Email, devices, **vault URLs** (leaked 2022) | Email/username, devices, Proton account linkage across services | ✅ None |
| Telemetry / analytics | Opt-in | Opt-out (no in-app toggle; Google Analytics opt-out tool only) | Opt-out | Opt-out | Opt-out | ✅ None - no INTERNET |
...
| PIN code unlock | ⚠️ Device passcode only (no in-app PIN) | ✅ | ✅ | ✅ | ✅ | ✅ |
| OCR (camera scan) | ❌ TOTP QR only (no card / doc OCR) | ✅ cards (iOS 2026.5.0+) | ✅ cards | ✅ credentials (Smart Scanner, May 2026) | ❌ | ✅ logins, cards, notes & custom fields |
| **Markdown in notes** | ✅ post-save render [^1p-md] | ❌ plain text only [^bw-md] | ❌ plain text only [^dl-md] | ❌ plain text only [^lp-md] | ❌ plain text only (open UserVoice request) [^pp-md] | ✅ **live-preview edit + view-mode render + helper bar** (user can also disable it in Settings) |
...
| Recycle bin (soft delete) | ✅ Recently Deleted (30 days) + Archive (hide, indefinite) | ✅ Trash | ❌ delete is immediate | ✅ 30-day | ✅ Trash | ✅ default on (user can toggle off in Settings) |
...
| Verifier brute-forceable from leaked file | ✅ Not possible - 128-bit Secret Key required off-device | If cloud breached | ❌ **Yes** - proven in 2026 breach for ~20 accounts | ❌ **Yes** - proven in 2022 breach for old-iteration vaults | If cloud breached | ✅ **Not possible** - hardware Keystore-bound (TEE / StrongBox) |
| Open source | ❌ Closed | ✅ Client + server | ⚠️ Source-available (CC BY-NC 4.0, not OSI-approved) | ⚠️ Closed (CLI tool GPL-2.0 only) | ⚠️ Clients only | ✅ Open source - GPL-3.0 |
...
[^dl2026]: Dashlane confirmed in June 2026 that attackers had stolen encrypted vaults for fewer than 20 personal-plan accounts. The attack (carried out in May 2026, disclosed in June 2026) used credentials reused from prior third-party breaches plus a brute-force on the 6-digit device-registration 2FA token to register rogue devices and download the encrypted vault payload. https://techcrunch.com/2026/06/02/password-manager-dashlane-says-hackers-stole-some-customers-password-vaults/
[^bw2026]: April 2026: Bitwarden's `@bitwarden/cli@2026.4.0` npm package was briefly compromised (~334 installs over ~90 minutes) as part of the Checkmarx Shai-Hulud supply-chain attack. Bitwarden published an advisory the same day; no vault data was accessed and the malicious version was pulled. https://tech.yahoo.com/cybersecurity/articles/bitwarden-confirms-compromise-10-million-185341045.html
[^lp2022]: LastPass 2022 breach: backup database exfiltrated via a two-stage employee laptop compromise (corporate laptop, then a senior DevOps engineer's home machine with malware that captured the master password). Customer names, emails, phone numbers, and vault URLs were stored unencrypted in the leaked database. UK ICO fined LastPass £1.2M (~$1.5M) in December 2025 for inadequate security, citing 1.6 million UK users affected. https://www.forbes.com/sites/daveywinder/2025/12/14/lastpass-data-breach---insufficient-security-exposed-16-million-users/
...
[^pp-md]: Proton Pass secure notes are plain text only; markdown rendering is an open feature request on Proton's UserVoice as of 2026 and has not shipped. https://protonmail.uservoice.com/forums/953584-proton-pass-authenticator/suggestions/46872187-markdown-support-in-notes
