...
| KDF | PBKDF2-650k + Secret Key [^1p-kdf] | PBKDF2-600k default; Argon2id (m=64 MiB, t=3, p=4) opt-in [^bw-kdf] | Argon2d (m=32 MiB, t=3, p=2) [^dl-kdf] | PBKDF2-600k (was 100,100 pre-breach) [^lp-kdf] | bcrypt (cost not disclosed) [^pp-kdf] | Argon2id (m=64 MiB, t=3, p=1) default; user-tunable up to m=256 MiB / t=16 [^1k-kdf] |
...
| --- | --- | --- | --- | --- |
| Password KDF (default) | PBKDF2-SHA256 650k iter + 128-bit Secret Key (~128 bits of extra entropy no KDF can match) | PBKDF2 600k (Argon2id 64MiB / t=3 / p=4 opt-in) | bcrypt cost 10 (≈100 ms, no memory hardness) | Argon2id m=64MiB / t=3 / p=1 default; user-tunable up to m=256 MiB / t=16 in Settings. Memory-hard, so stronger than bcrypt/PBKDF2 vs GPU/ASIC. At default, comparable to Bitwarden's opt-in Argon2id; at higher presets, exceeds it. 1Password's Secret Key remains unmatched against offline brute force. |
...
[^1k-kdf]: 1Key defaults to Argon2id with t=3, m=65,536 KiB (64 MiB), p=1, 256-bit output (the Standard preset). Users can switch to stronger fixed presets (Standard-plus m=64/t=8, Hardened m=128/t=4, Maximum m=128/t=8) or pick custom Argon2id parameters up to m=256 MiB / t=16 in Settings > Security > Encryption strength. See `app/src/main/java/com/onekey/core/security/KdfPreset.kt` and `CryptoManager.kt`.
